...
export default class EditorComponent extends Component {
  ...
  constructor(props) {
      ...
      const value = path(
          props.path.split('.').map((i) => (identical(NaN, +i) ? i : +i)),
          props,
      );
      ...
      const html = stateToHTML(editorState.getCurrentContent(), {
          inlineStyleFn: (styles) => {
              const textAlign = styles
                  .filter((value) => contains(value, ['center', 'left', 'right']))
                  .first();
  ...
  toggleInlineStyle = (style, inline) => {
      ...
      style === 'link'
          ? inline
              ? confirmLink(editorState, urlValue)
              : removeLink(editorState)
          : contains(style, ALIGNMENTS)
              ? styleWholeSelectedBlocksModifier(editorState, style, without([style], ALIGNMENTS))
              : inline
  ...
  blockStyleFn = (block) => {
      ...
      if (!block.getText()) {
          const previousBlock = this.state.editorState
              .getCurrentContent()
  ...
  render() {
      ...
      return isEditor ? (
          <div
              ...
              className={cx(styles.editorContainer, 'editor-container', {
                  [styles.block]: staticContent,
              })}
          >
        editor
          </div>
      ) : (
          <div className="editor-view" dangerouslySetInnerHTML={{ __html: this.props[path] }} />
      );
  }
}
